...
# MSG91 API details
API_KEY = '445027AfoRX678C67e5758eP1'  # Replace with your MSG91 API key
SENDER = 'qis'  # Replace with your Sender ID (you can get this after registration)

# Parent's mobile numbers stored in a dictionary (name -> number)
parent_numbers = {
    '22491A051L Prathyusha from 3rd year CSE-1': '7702407377',
    'Student2': '918765432109',
    # Add more students and their parent's numbers
}

# Template ID (Replace with your actual MSG91 template ID)
TEMPLATE_ID = '67e57814d6fc057f49210502'  # Replace this with your actual template ID
...
def mark_attendance(name):
    ...
    # Send SMS to Parent about attendance
    send_sms_to_parent(name)

# Function to send SMS using MSG91 API with Template ID
def send_sms_to_parent(student_name):
    if student_name in parent_numbers:
        parent_number = parent_numbers[student_name]
        ...
        data = {
            ...
            'template_id': TEMPLATE_ID,  # Using Template ID for SMS
            'numbers': parent_number,
            'message': f"Dear Parent, your child {student_name} has arrived at the college. Attendance has been marked."
        }

        # Send the request to MSG91 API
        try:
            response = requests.post("https://api.msg91.com/api/v5/apsms/send", data=data)
            response_data = response.json()

            if response_data.get('type') == 'success':
                print(f"SMS sent to {parent_number} successfully.")
            else:
                print(f"Failed to send SMS to {parent_number}: {response_data}")
        except Exception as e:
